...
@implementation APIEditShopCartManager
- (instancetype)init
{
    self = [super init];
    if (self) {
...
{
    return self.userId.length > 0 && self.productId.length > 0 && self.skuId.length > 0 && self.cartId.length > 0;
}
...
- (NSDictionary *)paramsForApi:(APIBaseManager *)manager
{
    return @{@"user_id" :self.userId?:@"",
             @"goods_id" :self.productId,
              @"sku_id" :self.skuId,
             @"cart_id" :self.cartId,
             @"goods_count" :NumberToString(self.count),
             };
}
