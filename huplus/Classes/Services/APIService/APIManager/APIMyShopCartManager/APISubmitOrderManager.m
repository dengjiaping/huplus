...
@implementation APISubmitOrderManager
- (instancetype)init
{
    self = [super init];
    if (self) {
...
- (NSDictionary *)paramsForApi:(APIBaseManager *)manager
{
    return @{@"user_id" :self.userId?:@"",
             ...
             @"cart_list" :self.cartIdListString?:@"",
             @"type" :self.submitType,
             @"sku_id" :self.skuId?:@"",
             @"store_id" :self.shopId?:@"",
             @"goods_num" :NumberToString(self.goodsCount)?:@""
             };
}
