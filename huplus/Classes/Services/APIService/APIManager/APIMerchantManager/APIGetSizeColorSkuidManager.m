...
@implementation APIGetSizeColorSkuidManager
- (instancetype)init
{
    self = [super init];
    if (self) {
...
{
    return self.productId.length > 0;
}
...
{
    return @{@"goods_id" :self.productId,
             @"size_id" :self.sizeId?:@"",
             @"color_id" :self.colorId?:@""
             };
}
